...
void main() {
    ...
    // ColorConfig
    vec3 mGreen = vec3(0.0, 1.0, 0.2);     // Green
    vec3 mDarkGreen = vec3(0.0, 0.1, 0.0); // GreenBackground
    vec3 mBlack = vec3(0.0, 0.0, 0.0);     // Black

    if (rawDepth >= 1.0) {
        // SkyAir：BlackDarkSpace
        ...
        if (max(max(abs(worldNormal.x), abs(worldNormal.y)), abs(worldNormal.z)) > 0.9) {
            worldPos.z += U_GameTime * U_GridSpeed; // OnlyhaveZMove
