...
public class DamageUtil {
   public static final DamageUtil.RaycastFactory HIT_FACTORY = (context, blockPos) -> {
      BlockState blockState = MeteorClient.mc.world.getBlockState(blockPos);
      return blockState.getBlock().getBlastResistance() < 600.0F
         ? null
         : blockState.getCollisionShape(MeteorClient.mc.world, blockPos).raycast(context.start(), context.end(), blockPos);
   };
   ...
   public static float explosionDamage(
      LivingEntity target, Vec3d targetPos, Box targetBox, Vec3d explosionPos, float power, DamageUtil.RaycastFactory raycastFactory
   ) {
      double modDistance = distance(
         targetPos.x, targetPos.y, targetPos.z, explosionPos.x, explosionPos.y, explosionPos.z
      );
      if (modDistance > power) {
         ...
         return calculateReductionsExplosion(damage, target, MeteorClient.mc.world.getDamageSources().explosion(null));
      }
   }
   ...
   private static float explosionDamage(LivingEntity target, LivingEntity predict, Vec3d explosionPos, float power, DamageUtil.RaycastFactory raycastFactory) {
      if (target == null) {
         return 0.0F;
      } else {
         return target instanceof PlayerEntity player && getGameMode(player) == GameMode.CREATIVE
            ...
            : explosionDamage(
               target,
               predict != null
                  ? new Vec3d(predict.getX(), predict.getY(), predict.getZ())
   ...
   public static DamageUtil.RaycastFactory getOverridingHitFactory(BlockPos overridePos, BlockState overrideState) {
      return (context, blockPos) -> {
         ...
         return blockState.getCollisionShape(MeteorClient.mc.world, blockPos).raycast(context.start(), context.end(), blockPos);
      };
   }
            ...
            BlockHitResult raycastResult = MeteorClient.mc
               ...
               .raycast(
                  new RaycastContext(
                     ...
                     new Vec3d(entity.getX(), MeteorClient.mc.world.getBottomY(), entity.getZ()),
                     ShapeType.COLLIDER,
                     FluidHandling.WATER,
                     entity
                  )
   ...
   private static float fallDamageReductions(LivingEntity entity, int surface) {
      ...
      return calculateReductions(fallHeight, entity, MeteorClient.mc.world.getDamageSources().fall());
   }
   ...
   private static float getExposure(Vec3d source, Box box, DamageUtil.RaycastFactory raycastFactory) {
      if ((Boolean)GlobalSetting.INSTANCE.optimizedCalc.get()) {
         int miss = 0;
         int hit = 0;
         ...
         for (int k = 0; k <= 1; k++) {
            for (int l = 0; l <= 1; l++) {
               for (int m = 0; m <= 1; m++) {
                  ...
                  if (raycast(vec3d, source, true) == net.minecraft.util.hit.HitResult.Type.MISS) {
                     miss++;
                  }
            ...
            for (double x = startX; x <= endX; x += xStep) {
               for (double y = startY; y <= endY; y += yStep) {
                  for (double z = startZ; z <= endZ; z += zStep) {
                     ...
                     if (raycast(new DamageUtil.ExposureRaycastContext(position, source), raycastFactory) == null) {
                        misses++;
                     }
   ...
   public static BlockHitResult raycast(DamageUtil.ExposureRaycastContext context, DamageUtil.RaycastFactory raycastFactory) {
   ...
   public static double squaredDistanceTo(double x, double y, double z) {
      return squaredDistance(
         MeteorClient.mc.player.getX(), MeteorClient.mc.player.getY(), MeteorClient.mc.player.getZ(), x, y, z
      );
   }
   ...
   @FunctionalInterface
   public interface RaycastFactory extends BiFunction<DamageUtil.ExposureRaycastContext, BlockPos, BlockHitResult> {
   }
}
