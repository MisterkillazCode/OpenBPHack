...
public class AlienBreakManager {
   ...
   public final ConcurrentHashMap<Integer, AlienBreakManager.BreakData> breakMap = new ConcurrentHashMap<>();
   public final ConcurrentHashMap<Integer, AlienBreakManager.BreakData> doubleMap = new ConcurrentHashMap<>();
   ...
   private void onTick(Post event) {
      if (mc.world != null && mc.player != null) {
         if (this.detectDouble) {
            for (int i : this.doubleMap.keySet()) {
               AlienBreakManager.BreakData breakData = this.doubleMap.get(i);
               if (breakData == null
                  || breakData.getEntity() == null
                  || mc.world.isAir(breakData.pos)
                  || breakData.timer.passedMs(Math.max(this.minTimeout * 1000.0, breakData.breakTime * this.doubleMineTimeout))) {
                  this.doubleMap.remove(i);
               }
            }
         }

         for (AlienBreakManager.BreakData breakData : this.breakMap.values()) {
            ...
            } else if (!breakData.complete && breakData.timer.passedMs(breakData.breakTime * this.breakTimeout)) {
               breakData.fade.setLength(0L);
               breakData.failed = true;
            } else {
   ...
   private void onPacket(Receive event) {
      if (mc.world != null && mc.player != null) {
         if (event.packet instanceof BlockBreakingProgressS2CPacket packet) {
            ...
            AlienBreakManager.BreakData breakData = new AlienBreakManager.BreakData(packet.getPos(), packet.getEntityId(), false);
            ...
            if (this.detectDouble && packet.getProgress() != 255) {
               if (packet.getProgress() != 0) {
                  AlienBreakManager.BreakData doublePos = this.doubleMap.get(packet.getEntityId());
                  if (doublePos != null) {
                     doublePos.pos = packet.getPos();
                     doublePos.timer.reset();
                  } else if (!unbreakable(packet.getPos())) {
                     this.doubleMap.put(packet.getEntityId(), new AlienBreakManager.BreakData(packet.getPos(), packet.getEntityId(), true));
                  }
            ...
            AlienBreakManager.BreakData current = this.breakMap.get(packet.getEntityId());
            if (current != null && !current.failed && current.pos.equals(packet.getPos())) {
               return;
            }

            this.breakMap.put(packet.getEntityId(), breakData);
            if (this.detectDouble && !this.doubleMap.containsKey(packet.getEntityId()) && !unbreakable(packet.getPos())) {
               this.doubleMap.put(packet.getEntityId(), new AlienBreakManager.BreakData(packet.getPos(), packet.getEntityId(), true));
            }
         }
      }
   ...
   public boolean isMining(BlockPos pos, boolean self) {
      for (AlienBreakManager.BreakData breakData : this.breakMap.values()) {
         if (breakData.getEntity() != null && !breakData.failed && breakData.pos.equals(pos)) {
            return true;
         }
      }
   ...
   public static int getTool(BlockPos pos) {
      ...
      RegistryEntry<Enchantment> effEntry = mc.world.getRegistryManager().getOrThrow(RegistryKeys.ENCHANTMENT).getOrThrow(Enchantments.EFFICIENCY);

      for (Entry<Integer, ItemStack> entry : AlienInventoryUtil.getInventoryAndHotbarSlots().entrySet()) {
   ...
   public static class BreakData {
      ...
      public BreakData(BlockPos pos, int entityId, boolean extraBreak) {
         ...
         this.breakTime = Math.max(AlienBreakManager.getBreakTime(pos, extraBreak), 50.0);
         this.fade = new AlienFadeUtils((long)this.breakTime);
         this.timer = new AlienTimer();
      }

      public Entity getEntity() {
         if (AlienBreakManager.mc.world == null) {
            return null;
         } else {
            Entity entity = AlienBreakManager.mc.world.getEntityById(this.entityId);
