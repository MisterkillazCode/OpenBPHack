...
public class AlienDamageUtils {
   ...
   public static float explosionDamage(LivingEntity target, Vec3d explosionPos, float power) {
      if (target instanceof PlayerEntity player && getGameMode(player) == GameMode.CREATIVE) {
         return 0.0F;
      } else {
      ...
      if (attacker instanceof PlayerEntity playerEntity) {
         ...
         if (weapon.getItem() instanceof MaceItem) {
            ...
            if (bonus > 0.0F) {
               ...
               if (density > 0) {
                  bonus = (float)(bonus + 0.5 * attacker.fallDistance);
               }
         ...
         if (charge > 0.9F
            && (attacker.fallDistance > 0.0 || attacker == mc.player && isCriticalsOn())
   ...
   private static boolean isCriticalsOn() {
      return mc.player != null && mc.player.fallDistance > 0.0 && !mc.player.isOnGround();
   }
   ...
   private static net.minecraft.util.hit.HitResult.Type raycast(Vec3d start, Vec3d end) {
      return (net.minecraft.util.hit.HitResult.Type)BlockView.raycast(start, end, null, (ctx, blockPos) -> {
   ...
   private static int getEnchantmentLevel(Object2IntMap<RegistryEntry<Enchantment>> enchantments, RegistryKey<Enchantment> key) {
      ...
      while (var2.hasNext()) {
         ...
         if (((RegistryEntry)entry.getKey()).matchesKey(key)) {
   ...
   private static void getEnchantments(ItemStack stack, Object2IntMap<RegistryEntry<Enchantment>> map) {
      ...
      if (!stack.isEmpty()) {
         ...
         if (stack.getItem() == Items.ENCHANTED_BOOK) {
            ench = (ItemEnchantmentsComponent)stack.get(DataComponentTypes.STORED_ENCHANTMENTS);
         } else {
         ...
         if (ench != null) {
            for (Entry<RegistryEntry<Enchantment>> entry : ench.getEnchantmentEntries()) {
               map.put((RegistryEntry)entry.getKey(), entry.getIntValue());
            }
         }
      }
   }
